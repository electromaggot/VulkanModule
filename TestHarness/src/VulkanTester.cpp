//
// VulkanModule Test Application.cpp
//
...
//
void VulkanTester::Init()
{
	DrawableProperties demoSpecifier = {		// triangle, shader-defined vertices, colored per vertex
		.mesh = ShaderSets3Vertices,
		.shaders = { { VERTEX,	 "triangle-vert.spv" },		// Simplest example of a renderable.
					 { FRAGMENT, "tricolor-frag.spv" } },
		.name = "VulkanModule Test Harness"
	};
	...
	platform.RegisterForceRenderCallback(VulkanTester::ForceUpdateRender, this);
}
...
/*void VulkanTester::loadNextRenderable()
{
...
void VulkanTester::updateRender()
{
...
//
void VulkanTester::ForceUpdateRender(void* pOurself)
{
	VulkanTester* pSelf = static_cast<VulkanTester*>(pOurself);
	if (pSelf) {
...
//
void VulkanTester::prepareForMainLoop()
{
