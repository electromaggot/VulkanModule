...
extern struct Constants
{
	...
	const int DefaultWindowWidth	= 640;			// a generic 4 × 3 aspect ratio
	...
	// The above defines one starting size (a very small one) satisfying this test harness, but
	//	may not be platform-compatible.  Actual "window size" variables - and their values* -
	//	remain PLATFORM-DEPENDENT therefore reside in the iPlatform and OS Abstraction classes.
	// * - window size may subsequently change from value above, if app implements a means to
	//	save/retrieve size, or as user interacts via drag-resize, maximize, rotate device, etc.

	const int MaxSaneScreenWidth	= 7680 * 2;		// 8K * 2 values.  Simply sanity-
