...
vector<iRenderableBase*> RenderBatchManager::buildBatches(const vector<iRenderableBase*>& renderables)
{
	...
	for (auto* pRenderable : renderables) {
		if (pRenderable->isSelfManaged) {					// Separate self-managed renderables - things
			selfManagedRenderables.push_back(pRenderable);	//	like ImGui render last with their own state.
			continue;
		}
	...
	batches.reserve(pipelineGroups.size());					// Convert map to vector of batches.
...
void RenderBatchManager::recordBatches(VkCommandBuffer& commandBuffer, int iFrame,
									   const vector<iRenderableBase*>& selfManagedRenderables)
{
	VkPipeline lastBoundPipeline = VK_NULL_HANDLE;

	for (const auto& batch : batches) {						// First: Record all batched renderables/scene objects:
		if (batch.key.pipeline != lastBoundPipeline) {	// Bind pipeline once per batch, not per renderable.
			vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, batch.key.pipeline);
			lastBoundPipeline = batch.key.pipeline;
		}

		for (auto* pRenderable : batch.renderables) {		// Draw all renderables in this batch:
			if (pRenderable->isSelfManaged) {		// Self-managed renderables should not be in batches, but check just in case.
			...
			if (renderable->IsSecondaryCommandBuffer()) { // If so, execute the pre-recorded command buffer.
				VkCommandBuffer secondaryCmdBuf = renderable->GetSecondaryCommandBuffer(iFrame);
				...
				concreteRenderable->IssueBindAndDrawCommands(commandBuffer, iFrame, true);	// Skip pipeline bind since we already
			}																				//	bound it once for the entire batch.
		}
	}
	...
	for (auto* pRenderable : selfManagedRenderables) {
		pRenderable->IssueBindAndDrawCommands(commandBuffer, iFrame);
	}
}
